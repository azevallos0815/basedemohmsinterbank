...
class BenefitStoreDetailActivity : BaseActivity(), OnMapReadyCallback {
    ...
    private lateinit var locationLiveData: CSLocationLiveData
    ...
    override fun onMapReady(googleMap: GoogleMap?) {
        ...
        val bitmapDescriptorFactory = CommonUtil.generateBitmapDescriptorFromRes(this, R.drawable.ic_store_marker_new)
        googleMap?.moveCamera(CameraUpdateFactory.newLatLngZoom(location, 15f))
        googleMap?.setInfoWindowAdapter(MapInfoWindowAdapter(LayoutInflater.from(this).inflate(R.layout.item_map_info, ctbToolbar, false)))
        //googleMap?.addMarker(MarkerOptions().icon(BitmapDescriptorFactory.fromBitmap(bitmap)).position(location!!))?.showInfoWindow()
        googleMap?.addMarker(MarkerOptions().icon(bitmapDescriptorFactory).position(location!!))?.showInfoWindow()
        googleMap?.setOnInfoWindowClickListener { showUpNavigation() }

        ctlAddress.onClickListener {
            val bounds = googleMap?.projection?.visibleRegion?.latLngBounds
            if (bounds?.contains(location) == false) {
                CameraUpdateFactory.newLatLngZoom(location, 15f)
                        .apply { googleMap.animateCamera(this) }
            }
        }
    }
